...
from helpers.io import AVC_PQ, PQ_DIR, SCROBBLE_PQ, dump_parquet, read_parquet, sanitize  # noqa: E402
...
def step2_extract_cluster_pairs(labels: np.ndarray, artist_names: list[str]) -> pd.DataFrame:
    ...
    log.info(
        "Non-singleton clusters: %d (sizes: min=%d, max=%d, median=%.0f)",
        len(groups),
        min(len(g) for g in groups),
...
def step4_build_parquet(verified_df: pd.DataFrame) -> pd.DataFrame:
    ...
    log.info(
        "gs_mb_dbscan.parquet: %d pairs (pos=%d, neg=%d) in WRatio [%d, %d).",
        len(filtered),
        pos,
        neg,
        WRATIO_LOWER,
...
def add_all_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    interaction_rows = df.apply(
        lambda r: pd.Series(
            compute_interaction_features(compute_pair_features(str(r["variant_a"]), str(r["variant_b"])))
        ),
        axis=1,
    )
