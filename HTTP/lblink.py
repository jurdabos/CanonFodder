...
LB_USER: Optional[str] = os.getenv("LB_USER")
...
def fetch_scrobbles_since(
    ...
    Paginates via ``max_ts`` (the LB API returns listens *before* max_ts).
    When *since* is None, fetches the complete listening history.
    Returns a DataFrame with columns ``Artist, Song, Album, uts, artist_mbid``
    — the same shape that ``normalise_scrobble_df`` expects.
    """
    client = _RequestsBackend()
    all_listens: list[dict[str, Any]] = []
    max_ts: int | None = None
    page = 0
    while True:
        page += 1
        if progress_callback:
            progress_callback(page, page, f"Fetching ListenBrainz page {page}")
        batch = client.get_listens(username, min_ts=since, max_ts=max_ts, count=1000)
        if not batch:
            break
        all_listens.extend(batch)
        # Pagination: next request fetches listens older than the oldest in this batch
        oldest_ts = min(listen["listened_at"] for listen in batch)
        if since is not None and oldest_ts <= since:
            break
        max_ts = oldest_ts
        # Fewer than requested → no more pages
        if len(batch) < 1000:
            break
        time.sleep(0.3)
    if not all_listens:
        return pd.DataFrame(columns=["Artist", "Song", "Album", "uts", "artist_mbid"])
    # Normalising into the canonical column set
