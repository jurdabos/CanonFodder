    ...
    with experiment.start_run(run_name=f"{model_name}_tuned", nested=True):
        ...
        for fold_idx, (train_idx, val_idx) in enumerate(skf.split(X_train, y_train)):
            ...
            experiment.log_cv_fold(
                fold_idx,
                metrics,
                run_name_prefix=f"{model_name}_tuned_fold",
            )
        ...
        log.info(
            "%s_tuned → AUC=%.4f | opt P=%.4f F1=%.4f (thr=%.3f) | hiP P=%.4f F1=%.4f (thr=%.3f)",
            model_name,
            auc,
            optimal_m["precision"],
            optimal_m["f1"],
...
def run_tuning(
    *,
    ...
    if not model_names:
        raise RuntimeError(f"No tunable models selected. Choose from: {sorted(_TUNABLE_MODELS)}")
    ...
    with experiment.start_run(run_name=parent_name):
        ...
        for model_name in model_names:
            ...
            skip_keys = {"is_unbalance", "eval_metric", "device", "random_state", "n_jobs", "verbosity"}
            best_params = {k: v for k, v in best.params.items() if k not in skip_keys}
    ...
    score = (
        0.4 * results[best_model]["hiprec_prec"]
        + 0.3 * results[best_model]["hiprec_f1"]
        + 0.3 * results[best_model]["auc"]
    )
