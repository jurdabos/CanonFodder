...
class CliInterface:
    ...
    def _show_main_menu(self):
        ...
        try:
            scrobble_stats, artist_info_stats = get_db_statistics()

            # Calculate the middle of the screen for layout
            middle_x = w // 2

            # Display Scrobble statistics table (left side)
            if scrobble_stats is not None and not scrobble_stats.empty:
