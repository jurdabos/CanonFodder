...
class TestSiameseTCNForward:
    ...
    def test_output_shape(self):
        ...
        model = SiameseTCN(
            vocab_size=50, embed_dim=16, tcn_channels=[32, 32],
            kernel_size=3, tcn_dropout=0.1, fc_dropout=0.1,
        )
        model.eval()
        batch = 4
        seq_len = 20
...
class TestHybridTCNForward:
    ...
    def test_skip_connection(self):
        ...
        model = HybridTCN(
            vocab_size=50, embed_dim=16, tcn_channels=[32, 32],
            kernel_size=3, tcn_dropout=0.1, fc_dropout=0.1, n_features=10,
        )
        model.eval()
...
class TestPredictSiamese:
    ...
    def test_returns_probabilities(self):
        ...
        model = SiameseTCN(
            vocab_size=vocab.size, embed_dim=8, tcn_channels=[16],
            kernel_size=3, tcn_dropout=0.0, fc_dropout=0.0,
        )
        model.eval()
        df = pd.DataFrame({
            "variant_a": ["abc", "xyz"],
...
class TestTrainSiamese:
    ...
    def test_runs_one_epoch(self):
        ...
        from corefunc.canon.tcn_trainer import (
            SiameseTCN, CharVocab, NamePairDataset, _train_siamese,
        )
        from torch.utils.data import DataLoader
        vocab = CharVocab()
        ...
        history = _train_siamese(
            model, loader, loader, y_val, pos_weight=1.0,
            epochs=2, lr=0.01, patience=5, device=device,
        )
...
class TestTrainHybrid:
    ...
    def test_runs_one_epoch(self):
        """Completes a single epoch without error."""
        from corefunc.canon.tcn_trainer import (
            HybridTCN, CharVocab, HybridDataset, _train_hybrid,
        )
        from torch.utils.data import DataLoader
        vocab = CharVocab()
        vocab.fit(["alpha", "beta", "gamma", "delta"])
...
class TestPrecisionBiasedObjective:
    ...
    def test_returns_float(self):
        ...
        score = _precision_biased_objective(
            trial, "LightGBM", X, y, ["f1", "f2"], spw, "cpu",
            n_folds=2, min_precision=0.5,
        )
    ...
    def test_unknown_model_raises(self):
        ...
        with pytest.raises(ValueError, match="Unknown model"):
            _precision_biased_objective(
                trial, "FakeModel", pd.DataFrame(), np.array([]),
                [], 1.0, "cpu",
            )
...
class TestExperimentRunnerCvEvaluate:
    ...
    def test_returns_mean_metrics(self, mock_exp):
        ...
        metrics = _cv_evaluate(
            clf, X, y, ["f1", "f2"], n_folds=3, random_state=42, model_name="RF",
        )
...
class TestSaveBestHistoricalModels:
    ...
    def test_filters_tuned_runs(self, mock_client_cls, _mock_uri):
        ...
        mock_run.data.metrics = {
            "opt_precision": 0.9, "auc": 0.95,
            "hiprec_precision": 0.85, "hiprec_f1": 0.8,
        }
