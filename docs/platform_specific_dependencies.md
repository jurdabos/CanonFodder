...
The project has migrated from Airflow to Prefect to avoid these conflicts. The Docker setup maintains Airflow for legacy compatibility. Note that `requirements.txt` is auto-generated by uv and kept for compatibility (e.g., Docker). uv remains the preferred tool for installing dependencies locally and handles platform markers automatically.
