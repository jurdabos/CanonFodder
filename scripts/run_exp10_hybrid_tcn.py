...
def step2_assemble_training(positives_all: pd.DataFrame) -> pd.DataFrame:
    ...
    log.info("Positive WRatio distribution: %s", dict(zip(
        [f"[{bin_edges[i]:.0f},{bin_edges[i+1]:.0f})" for i in range(n_bins)], pos_hist,
    )))
...
class HybridTCN(nn.Module):
    ...
    """
    def __init__(self, vocab_size, embed_dim, tcn_channels, kernel_size,
                 tcn_dropout, fc_dropout, n_features):
        super().__init__()
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        for x_a, x_b, feats, labels in train_loader:
            ...
            if torch.isnan(loss) or torch.isinf(loss):
                nan_batches += 1
                if nan_batches <= 3:
                    log.warning("NaN/Inf loss at epoch %d, batch %d. Restoring weights.", epoch, n_batches + nan_batches)
...
def main():
    ...
    train_df, val_df = train_test_split(
        train_full, test_size=0.15, stratify=train_full["to_link"], random_state=RANDOM_STATE,
    )
